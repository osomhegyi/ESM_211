...
w_pop_data_philippines<- w_pop_data |> clean_names() |>
  filter(country_name=="Philippines") |> # filter for Namibia
  select(year, population) |> drop_na()
```
...
human_ts_model_trans<-ggplot(w_pop_data, aes(x=year, y=population))+
  ...
  ggtitle("Human Population in Namibia (1950-2022)")+
  theme_bw()
human_ts_model_trans # pretty good model fit, but the data is not quite linear. The model over estimate and under estimate often, but provides a general idea of the data trend even if it is not the best fit.
...
human_ts_model<-ggplot(w_pop_data, aes(x=year, y=population))+
  ...
  ggtitle("Human Population in Namibia (1950-2022)")+
  theme_bw()
human_ts_model # good model fit until about year 1988, then the data deviates a bit from the model fit. Recent years (~2008-2022) aren't well fit for this model, but the first several years are very well fit.
```
